...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag{
  // 让控件真身过去!
    ...
    // MARK: - 让控制器给用户提示
    // MARK: - 判断并执行
    int num1 = arc4random_uniform(9)+1;
    int num2 = arc4random_uniform(9)+1;
    int num3 = arc4random_uniform(9)+1;
    int num4 = arc4random_uniform(9)+1;
    int num5 = arc4random_uniform(9)+1;
    if ([self.delegate respondsToSelector:@selector(MMRotateView:didFinishChooseNumber:)]) {
        [self.delegate MMRotateView:self didFinishChooseNumber:[NSString stringWithFormat:@"%d,%d,%d,%d,%d",num1,num2,num3,num4,num5]];
    }
}
//设置定时器开始转动
- (void)startRotating{
